...
public class FilmeService {
	...
	public FilmeDTO obter(Long idFilme) {
		return modelMapper.map(repository.findById(idFilme).get(), FilmeDTO.class);
	}
	
	public List<FilmeDTO> buscarFilesPorAno(Integer ano, Pageable pageable) {
		return repository.buscarFilesPorAno(ano, pageable).stream()
	...
	public List<FilmeDTO> buscarFilesPorGenero(Integer idGenero, PageRequest pageable) {
		return repository.buscarFilesPorGenero(idGenero, pageable).stream()
		          .map(f -> modelMapper.map(f, FilmeDTO.class))
		          .collect(Collectors.toList());
	}

	public void remover(FilmeDTO filme) {
		Filme entidade = modelMapper.map(filme, Filme.class);
